...
const NavbarComponent = () => {
  ...
  return (
    <nav
      ref={navbarRef}
      className="bg-gradient-to-r from-red-700 to-red-600 text-white shadow-md sticky top-0 z-50"
    >
      ...
      <div className={`${isMenuOpen ? 'block' : 'hidden'} xl:block xl-4 xl:mt-0 text-lg font-bold px-6 pb-2`}>
        {authUser && authUser.esAdmin === 1 ? (
            ...
            <div className="flex flex-col md:flex-row md:items-center md:gap-6">
              ...
              <div className="relative">
                ...
                >
                  ...
                  Inventarios
                  <svg
                    className={`w-4 h-4 transition-transform ${openDropdown === 'inventarios' ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <div className="relative">
                ...
                >
                  Ventas
                  <svg
                    className={`w-4 h-4 transition-transform ${openDropdown === 'ventas' ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                >
                  <Link to="/admin/estados_ventas" className="block px-4 py-2 md:text-gray-800 hover:bg-red-800 md:hover:bg-gray-100 rounded transition-colors">
                    Estados ventas
                  </Link>
                  <Link to="/admin/ventas" className="block px-4 py-2 md:text-gray-800 hover:bg-red-800 md:hover:bg-gray-100 rounded transition-colors">
                    Ventas
                  </Link>
                  <Link to="/admin/estados_reservas" className="block px-4 py-2 md:text-gray-800 hover:bg-red-800 md:hover:bg-gray-100 rounded transition-colors">
                    Estados reservas
                  </Link>
                  <Link to="/admin/reservas" className="block px-4 py-2 md:text-gray-800 hover:bg-red-800 md:hover:bg-gray-100 rounded transition-colors">
                    Reservas
                  </Link>
                </div>
              </div>
          ...
          // VISTA USUARIO (sin cambios)
          <>
            {/* ...tu código de vista usuario igual... */}
          </>
        )}
      </div>
    </nav>
